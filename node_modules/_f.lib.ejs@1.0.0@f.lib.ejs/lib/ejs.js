...
Template.prototype = {
  ...
  generateSource: function () {
    ...
    if (matches && matches.length) {
      matches.forEach(function (line, index) {
        ...
        // HACK: backward-compat `include()` preprocesssor directives
        if ((include = line.match(/include\(([\s\S]+)\)\s+$/))) {
          opening = matches[index - 1];
          // Must be in EVAL or RAW mode
          if (opening && (opening == '<' + d || opening == '<' + d + '-' || opening == '<' + d + '_')) {
            var ss = include[1].split(',');
            var path = ss.shift().trim().replace(/^["']{1}(.+)["']{1}$/, "$1");
            var params = ss.join(',');
            params = params.length > 0 ? ', ' + params : '';
            includeOpts = utils.shallowCopy({}, self.opts);
            includeObj = includeSource(path, includeOpts);
            if (self.opts.compileDebug) {
              includeSrc =
                  '    ; (function(locals){' + '\n'
                  + '      var __line = 1' + '\n'
  ...
  scanLine: function (line) {
    ...
    var include = function (path, includeData) {
      var d = utils.shallowCopy({}, includeData);
      return includeFile(path, self.opts);
    };
